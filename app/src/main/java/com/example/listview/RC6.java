...
public class RC6 {
    ...
    private int[] convByteWords(byte[] key, int u, int c) {
        int[] tmp = new int[c];
    ...
    //Penjadwalan Kunci RC6 Mencapurkan Array S Dengan L
    private int[] generateSubKeys(byte[] key) {
        int u = w / 8;
        int c = key.length / u;
        int t = 2 * r + 4;

        int[] L = convByteWords(key, u, c);
        int[] S = new int[t];
        S[0] = Pw;
    ...
    //Prosses Melakukan Enkripsi RC6
    ...
    //Prosses Melakukan Dekripsi RC6
    ...
    //Prosses Penghilangan padding pada Key
    ...
    //Memecah Bloc Cihper Text Ke Dalam 4 Register
    ...
    //Prosses Padding Awal Dalam Penjadwaln Kunci
    ...
    private int rotl(int val, int pas) {
        return (val << pas) | (val >>> (3-pas));
    }

    //Memecah Block ChiperText Ke Dalam 4 Register
